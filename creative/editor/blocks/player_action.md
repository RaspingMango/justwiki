...
[^28]: **Тип атрибута** `attribute_type`:
...
[^33]: **Выпадение предметов** `death_drops`:
...
[^35]: **Режим столкновения** `collidable`:
...
[^38]: **Мгновенное возрождение** `instant_respawn`:
...
[^39]: **Оставить текущий поворот** `keep_rotation`:
...
[^40]: **Сохранение инерции** `keep_velocity`:
...
[^41]: **Спешиться после телепортации** `dismount`:
...
[^42]: **Учитывать текущую инерцию** `increment`:
...
[^45]: **Полёт на элитрах** `is_gliding`:
...
[^49]: **Направление удара** `direction`:
...
[^54]: **Тип отображения частиц** `unit_of_measurement`:
...
[^57]: **Видимость для игрока** `visible_to_self`:
...
[^58]: **Тип сервера скинов** `server_type`:
...
[^63]: **Установить визуальный огонь** `visual_fire`:
