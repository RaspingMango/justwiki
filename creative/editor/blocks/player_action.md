...
[^20]: **Перезаписывать существующие эффекты** `overwrite`:
...
[^21]: **Показывать иконку эффекта** `show_icon`:
...
[^22]: **Показывать частицы** `particle_mode`:
...
[^23]: **Блокировка состояния (время не будет изменяться)** `ticking_locked`:
...
[^27]: **Тип атрибута** `attribute_type`:
