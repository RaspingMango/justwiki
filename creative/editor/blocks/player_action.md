...
[^25]: **Тип атрибута** `attribute_type`:
...
[^30]: **Выпадение предметов** `death_drops`:
...
[^32]: **Режим столкновения** `collidable`:
...
[^35]: **Мгновенное возрождение** `instant_respawn`:
...
[^36]: **Оставить текущий поворот** `keep_rotation`:
...
[^37]: **Сохранение инерции** `keep_velocity`:
...
[^38]: **Спешиться после телепортации** `dismount`:
...
[^39]: **Учитывать текущую инерцию** `increment`:
...
[^42]: **Полёт на элитрах** `is_gliding`:
...
[^45]: **Направление удара** `direction`:
...
[^50]: **Тип отображения частиц** `unit_of_measurement`:
...
[^53]: **Видимость для игрока** `visible_to_self`:
...
[^54]: **Тип сервера скинов** `server_type`:
...
[^59]: **Установить визуальный огонь** `visual_fire`:
